        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Collection Rate',   value: `${collectedPct}%`,               color: collectedPct >= 80 ? 'text-green-700' : 'text-orange-600', bg: 'bg-surface' },
            ].map(k => (
              ...
              <table className="w-full min-w-[900px] text-sm">
                ...
                <tbody className="divide-y divide-border-default">
                  {data?.rows.map(r => {
                    ...
                    return (
                      <tr key={r.bookingId} className={r.overdueInstallments > 0 ? 'bg-red-50/30' : 'hover:bg-surface-muted'}>
                        ...
                        <td className="px-3 py-2 w-28">
                          <div className="flex items-center gap-1.5">
                            <div className="flex-1 h-1.5 bg-surface-muted rounded-full overflow-hidden">
                              <div className={`h-1.5 rounded-full ${p >= 100 ? 'bg-green-500' : p >= 60 ? 'bg-primary' : 'bg-orange-500'}`}

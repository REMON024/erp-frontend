            ...
            {Object.entries(byCategory).map(([cat, items]) => {
              ...
              return (
                <div key={cat} className="bg-surface rounded-xl border border-border-default overflow-hidden">
                  <div className={`px-4 py-2.5 border-b flex items-center justify-between ${isOver ? 'bg-danger/10 border-danger/20' : 'bg-surface-muted border-border-default'}`}>
                    <div className="flex items-center gap-2">
                      {isOver ? <TrendingUp className="w-4 h-4 text-danger" /> : <TrendingDown className="w-4 h-4 text-success" />}
                    ...
                    <div className="flex items-center gap-4 text-xs">
                      ...
                      {catAct > 0 && <span className={isOver ? 'text-danger font-semibold' : 'text-success font-semibold'}>Act: {fmt(catAct)}</span>}
                      {catVar !== 0 && <span className={`font-bold ${isOver ? 'text-danger' : 'text-success'}`}>{catVar > 0 ? '+' : ''}{fmt(catVar)}</span>}
                    </div>
                  </div>
                  <table className="w-full min-w-[640px] text-xs">
                    ...
                    <tbody className="divide-y divide-border-default">
                      {items.map((item, idx) => {
                        ...
                        return (
                          <tr key={idx} className={over ? 'bg-danger/10 hover:bg-danger/10' : 'hover:bg-surface-muted'}>
                            ...
                            <td className="px-3 py-2 text-right">
                              {item.actualAmount > 0
                                ? <span className={over ? 'text-danger font-semibold' : 'text-success'}>{fmt(item.actualAmount)}</span>
                                : <span className="text-content-muted/50">—</span>}
                            </td>
                            <td className={`px-3 py-2 text-right font-semibold ${over ? 'text-danger' : item.variance < 0 ? 'text-success' : 'text-content-muted'}`}>
                              {item.variance !== 0 ? `${item.variance > 0 ? '+' : ''}${fmt(item.variance)}` : '—'}
                            </td>
                            <td className={`px-3 py-2 text-right ${over ? 'text-danger' : item.variancePct < 0 ? 'text-success' : 'text-content-muted'}`}>

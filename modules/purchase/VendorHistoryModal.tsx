        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-xl border border-border-default bg-primary/10 p-4">
              ...
              <p className="text-2xl font-bold text-info mt-1">{fmt(data.totalOrderValue)}</p>
            </div>
            <div className="rounded-xl border border-border-default bg-success/10 p-4">
              <p className="text-xs text-success uppercase tracking-wide">Total Paid</p>
              <p className="text-2xl font-bold text-success mt-1">{fmt(data.totalPaid)}</p>
            </div>
            <div className={`rounded-xl border p-4 ${data.outstandingPayable > 0 ? 'bg-danger/10 border-danger/20' : 'bg-surface-muted border-border-default'}`}>
              <p className={`text-xs uppercase tracking-wide ${data.outstandingPayable > 0 ? 'text-danger' : 'text-content-muted'}`}>Outstanding</p>
              <p className={`text-2xl font-bold mt-1 ${data.outstandingPayable > 0 ? 'text-danger' : 'text-content'}`}>{fmt(data.outstandingPayable)}</p>
            </div>
          </div>
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full min-w-[560px] text-sm">
                    <thead className="bg-surface-muted border-b border-border-default">
                      <tr>{[{ h: 'PO No.' }, { h: 'Date' }, { h: 'Project' }, { h: 'Amount', num: true }, { h: 'Status' }].map(({ h, num }) => (
                    ...
                    <tbody className="divide-y divide-border-default">
                      {data.purchaseOrders.map(po => (
                        <tr key={po.id} className="hover:bg-surface-muted">
                          ...
                          <td className="px-4 py-2.5 font-semibold text-content text-right tabular-nums">{fmt(po.totalAmount)}</td>
                    ...
                    <tfoot className="border-t border-border-default bg-surface-muted">
                      <tr>
                        ...
                        <td className="px-4 py-2 font-bold text-content text-right tabular-nums">{fmt(data.totalOrderValue)}</td>
                        <td />
                      </tr>
                    </tfoot>
                  </table>
            ...
            {data.payments.length === 0
              ...
              : <div className="overflow-x-auto">
                  <table className="w-full min-w-[500px] text-sm">
                    <thead className="bg-surface-muted border-b border-border-default">
                      <tr>{[{ h: 'Payment No.' }, { h: 'Date' }, { h: 'Amount', num: true }, { h: 'Method' }, { h: 'Reference' }].map(({ h, num }) => (
                        <th key={h} className={`px-4 py-2 text-xs font-semibold text-content-muted ${num ? 'text-right' : 'text-left'}`}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody className="divide-y divide-border-default">
                      {data.payments.map(p => (
                        <tr key={p.id} className="hover:bg-surface-muted">
                          ...
                          <td className="px-4 py-2.5 font-semibold text-success text-right tabular-nums">{fmt(p.amount)}</td>
                          <td className="px-4 py-2.5">
                    ...
                    <tfoot className="border-t border-border-default bg-surface-muted">
                      <tr>
                        ...
                        <td className="px-4 py-2 font-bold text-success text-right tabular-nums">{fmt(data.totalPaid)}</td>
                        <td colSpan={2} />
                      </tr>
                    </tfoot>
                  </table>

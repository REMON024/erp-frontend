...
export interface Resource {
  ...
  category?: string; unit: string; status: string; description?: string
...
function ResourceModal({ resource, defaultType, onClose, onSaved }: {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<Form>({
    ...
    defaultValues: resource ?? {
      status: 'Active', minimumStock: 0, averageCost: 0, resourceType: defaultType,
    },
  })
      ...
      <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input {...register('category')} className={inp} placeholder="Cement / Plant / Testing / Civil" />
          </div>
        </div>
...
export function ResourceMasterPage() {
  ...
  const { data: resources = [], isLoading, error, refetch } = useApiData<Resource[]>({
    url: '/resources',
    params: { search: search || undefined, types: tab === 'All' ? undefined : tab },
    queryKey: ['resources', search, tab],
  })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {TYPES.map(t => {
          ...
          return (
            <button key={t} onClick={() => setTab(tab === t ? 'All' : t)}
      ...
      <SearchBar value={search} onChange={setSearch} placeholder="Search resource name or code…" onRefresh={refetch}>
        <Select value={tab} onChange={e => setTab(e.target.value as ResourceType | 'All')} className="min-w-[150px]">
